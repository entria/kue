...
function Job( type, data, id ) {
  this.id            = id;
  this.saved         = false; // Flag to indicate whether this been saved or not
  this.type          = type;
  this.data          = data || {};
...
Job.prototype.save = function( fn ) {
  ...
  // update
  if( this.id && this.saved ) return this.update(fn);

  // incr id
  client.incr(client.getKey('ids'), function( err ) {// function( err, id ) {
    if( err ) return fn(err);
